...
function T(key, params) {
    if (typeof window !== 'undefined' && window.ZeroJackpotI18n && window.ZeroJackpotI18n.t) {
...
function attachSlotListeners() {
    document.querySelectorAll('.number-slot').forEach(function (slot) {
...
function openNumberPicker(slotIndex, type) {
    ...
    title.textContent = type === 'main' ? T('picker.titleMain') : T('picker.titleStar');
...
function selectNumber(number) {
    ...
    const mainValid = selectedMainNumbers.every(function (n) {
        return n !== null;
    });
    const starsValid = selectedStarNumbers.every(function (n) {
        return n !== null;
    });
...
function updateSlotDisplay() {
    selectedMainNumbers.forEach(function (num, index) {
    ...
    selectedStarNumbers.forEach(function (num, index) {
...
function attachEventListeners() {
    ...
    document.getElementById('sim100').addEventListener('click', function () {
        handleSimulation(100);
    });
    document.getElementById('sim1000').addEventListener('click', function () {
        handleSimulation(1000);
    });
    document.getElementById('sim10000').addEventListener('click', function () {
        handleSimulation(10000);
    });
    document.getElementById('sim100000').addEventListener('click', function () {
        handleSimulation(100000);
    });
    document.getElementById('sim1000000').addEventListener('click', function () {
        handleSimulation(1000000);
    });
    document.getElementById('sim10000000').addEventListener('click', function () {
        handleSimulation(10000000);
    });
...
function handleGenerateRow() {
    ...
    const finalMainNumbers = row.main.slice();
    const finalStarNumbers = row.stars.slice();
    ...
    mainSlots.forEach(function (slot) {
        slot.classList.remove('empty');
    });
    starSlots.forEach(function (slot) {
        slot.classList.remove('empty');
    });
    ...
    const interval = setInterval(function () {
        ...
        if (progress < 0.9) {
            mainSlots.forEach(function (slot) {
                ...
                slot.textContent = String(randomNum);
            });
            ...
            mainSlots.forEach(function (slot, index) {
                slot.textContent = String(finalMainNumbers[index]);
            });

            starSlots.forEach(function (slot, index) {
                slot.textContent = String(finalStarNumbers[index]);
            });
...
function updateSimulationButtons() {
    ...
    if (isValid) {
        currentPlayerRow = {
            main: selectedMainNumbers.slice().sort(function (a, b) {
                return a - b;
            }),
            stars: selectedStarNumbers.slice().sort(function (a, b) {
                return a - b;
            })
        };
    } else {
...
async function handleSimulation(numberOfDraws) {
    ...
    const costPerRow = getTicketPrice();
...
function displayResults(simulation, numberOfDraws, costPerRow) {
    ...
    const winsByCategory = simulation.winsByCategory;
...
function displayWinsByCategory(winsByCategory, totalDraws) {
    ...
    getPrizeTiers().forEach(function (tier) {
        ...
        row.innerHTML =
            ...
            tier.name +
            '</td><td>' +
            formatNumber(categoryData.count) +
            ' (' +
            probability +
            '%)</td><td>' +
            formatCurrency(categoryData.prize) +
            '</td><td style="' +
            (categoryData.totalPrize > 0 ? 'font-weight: 700; color: var(--success)' : '') +
            '">' +
    ...
    const noWinCount =
        totalDraws -
        Object.values(winsByCategory).reduce(function (sum, cat) {
...
function displayBestResult(bestResult) {
    ...
    bestResultTitle.textContent = T('sim.bestDrawTitle', { n: String(bestResult.drawNumber) });

    const oddsText =
        category && category.odds
            ? ' | <strong>' + T('sim.oddsLabel') + '</strong> ' + category.odds
            : '';

    bestResultContent.innerHTML =
        '<div class="draw-display">' +
        bestResult.draw.main
            .map(function (num) {
                return (
                    '<div class="draw-number ' +
                    (bestResult.matchedMainNumbers.includes(num) ? 'match' : '') +
                    '">' +
                    num +
                    '</div>'
                );
        ...
        bestResult.draw.stars
            .map(function (num) {
                return (
                    '<div class="draw-number ' +
                    (bestResult.matchedStarNumbers.includes(num) ? 'match' : '') +
                    '">' +
                    num +
                    '</div>'
                );
...
function displayInvestmentComparison(numberOfDraws, costPerRow, lotteryNetResult) {
    ...
    descriptionElement.textContent = T('sim.investDesc', {
        amount: formatCurrency(dailyInvestment),
    ...
    comparisonLabel.textContent = T('sim.diffVsLotto');
    if (difference > 0) {
...
window.__zjAfterI18n = function () {
    if (document.getElementById('mainSlots')) {
        initializeApp();
    }
};
